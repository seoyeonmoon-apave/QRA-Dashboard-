...
if uploaded_image is not None and uploaded_excel is not None:
    ...
    with col_tool:
        ...
        if drawing_type == "📍 누출점 (Point)":
            drawing_mode = "point"; stroke_color = "red"; fill_color = "rgba(255, 0, 0, 0.5)"
        elif drawing_type == "🟦 작업구역 (Rect)":
            drawing_mode = "rect"; stroke_color = "blue"; fill_color = "rgba(0, 0, 255, 0.3)"
        else:
            drawing_mode = "transform"; stroke_color = "black"; fill_color = "rgba(0, 0, 0, 0.3)"
        ...
        if points or rects:
            ...
            with col_map_img:
                ...
                def draw_transparent_label(img, text, x, y, color):
                    font = cv2.FONT_HERSHEY_DUPLEX; font_scale = 0.5; thickness = 1
                ...
                for i, pt in enumerate(points):
                    cx, cy = int(pt["left"] + pt["radius"]), int(pt["top"] + pt["radius"])
                    draw_transparent_label(annotated_img, f"Point {i+1}", cx + 12, cy - 12, (255, 0, 0))

                for i, r in enumerate(rects):
                    rx, ry = int(r["left"]), int(r["top"])
            ...
            with col_rect:
                ...
                for i, r in enumerate(rects):
                    cx, cy = int(r["left"] + (r["width"] * r["scaleX"]) / 2), int(r["top"] + (r["height"] * r["scaleY"]) / 2)
