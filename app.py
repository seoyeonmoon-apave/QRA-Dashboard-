            ...
            if has_duplicate:
                ...
                if st.button("QRA 물리 모델 초정밀 연산 및 렌더링 실행", type="primary", use_container_width=True):
                    ...
                    try:
                        ...
                        progress_bar.progress(60, text="3/5: 인구 밀집도(IRPA) 및 F-N Curve 산출 중...")
                        ...
                        progress_bar.progress(95, text="5/5: 통합 PDF 리포트 병합 중...")
                        images_to_pdf = []
                        for img_array in [st.session_state.base_lsir, st.session_state.base_therm, st.session_state.base_exp, st.session_state.base_irpa]:
                            images_to_pdf.append(Image.fromarray(img_array))
                        if fn_data:
                            images_to_pdf.append(Image.open(io.BytesIO(st.session_state.bytes_fn)).convert("RGB"))
                        images_to_pdf.append(Image.open(io.BytesIO(st.session_state.bytes_irpa)).convert("RGB"))
                        
                        pdf_bytes = io.BytesIO()
                        images_to_pdf[0].save(pdf_bytes, format="PDF", resolution=100.0, save_all=True, append_images=images_to_pdf[1:])
                        st.session_state.pdf_report = pdf_bytes.getvalue()
                ...
                if st.session_state.get('qra_calculated', False):
                    ...
                    with tab1:
                        ...
                        with col_c:
                            ...
                            st.download_button("⬇️ 개별 다운로드", get_img_bytes(final_lsir), "LSIR_Contour.png", "image/png")
                    with tab2:
                        col_l, col_c, col_r = st.columns([0.05, 0.9, 0.05])
                        with col_c:
                            st.image(final_therm, use_container_width=True)
                            st.download_button("⬇️ 개별 다운로드", get_img_bytes(final_therm), "Thermal_Contour.png", "image/png")
                    with tab3:
                        col_l, col_c, col_r = st.columns([0.05, 0.9, 0.05])
                        with col_c:
                            st.image(final_exp, use_container_width=True)
                            st.download_button("⬇️ 개별 다운로드", get_img_bytes(final_exp), "Explosion_Contour.png", "image/png")
                    with tab4:
                        col_l, col_c, col_r = st.columns([0.05, 0.9, 0.05])
                        with col_c:
                            st.image(final_irpa, use_container_width=True)
                            st.download_button("⬇️ 개별 다운로드", get_img_bytes(final_irpa), "IRPA_Contour.png", "image/png")
                    with tab5:
                        col_l, col_c, col_r = st.columns([0.05, 0.9, 0.05])
                        with col_c:
                            if st.session_state.bytes_fn:
                                st.image(st.session_state.bytes_fn, use_container_width=True)
                                st.download_button("⬇️ 개별 다운로드", st.session_state.bytes_fn, "FN_Curve.png", "image/png")
                            else:
                    ...
                    with tab6:
                        ...
                        with col_c:
                            ...
                            st.download_button("⬇️ 개별 다운로드", st.session_state.bytes_irpa, "IRPA_Chart.png", "image/png")
